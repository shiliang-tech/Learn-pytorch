...
| 2 | 打开 `../lessons/lesson_19_cnn_synthetic_images.py` 补全 TODO | 把概念变成能运行的代码 |
| 3 | 运行练习文件，观察输出 | 用 shape、loss、accuracy 判断代码是否合理 |
| 4 | 最后对照 `../answers/answer_19_cnn_synthetic_images.py` | 查漏补缺，而不是直接抄答案 |
...
| 写法 | 什么时候用 |
| --- | --- |
| `Conv2d -> ReLU -> Pool -> Conv2d -> ReLU -> Pool -> Flatten -> Linear` | 典型 CNN 分类器：`Conv2d -> ReLU -> Pool -> Conv2d -> ReLU -> Pool -> Flatten -> Linear`。 |
| 概念 / 经验 | 小图像任务可以先用 8、16、32 个通道，不必一开始很大。 |
| 概念 / 经验 | 卷积层学习局部模式，池化层降低空间尺寸并扩大感受野。 |
| `nn.Sequential` | `nn.Sequential` 很适合写直线型 CNN。 |
| `channels * height * width` | Flatten 后 Linear 输入维度等于 `channels * height * width`。 |
| 概念 / 经验 | 如果不想手算维度，可以先跑一个 dummy input 打印 shape。 |
| 概念 / 经验 | 分类输出层维度等于类别数。 |
| 概念 / 经验 | CNN 输入一般要归一化到合理范围，例如 0-1 或均值方差标准化。 |
| 概念 / 经验 | 合成数据适合练流程，但真实图像还要考虑数据增强和 train/val/test 切分。 |
| 概念 / 经验 | 训练准确率 1.0 不代表泛化好，要看独立验证集。 |
...
- 练习文件：`../lessons/lesson_19_cnn_synthetic_images.py`
- 答案文件：`../answers/answer_19_cnn_synthetic_images.py`
