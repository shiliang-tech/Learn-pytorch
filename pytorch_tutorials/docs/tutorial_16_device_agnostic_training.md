...
| 2 | 打开 `../lessons/lesson_16_device_agnostic_training.py` 补全 TODO | 把概念变成能运行的代码 |
| 3 | 运行练习文件，观察输出 | 用 shape、loss、accuracy 判断代码是否合理 |
| 4 | 最后对照 `../answers/answer_16_device_agnostic_training.py` | 查漏补缺，而不是直接抄答案 |
...
| 写法 | 什么时候用 |
| --- | --- |
| `device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')` | 统一写 `device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')`。 |
| `model.to(device)` | 模型创建后调用 `model.to(device)`。 |
| `xb = xb.to(device); yb = yb.to(device)` | 每个 batch 进入模型前调用 `xb = xb.to(device); yb = yb.to(device)`。 |
| `torch.zeros_like(x)`<br>`device=device` | 新创建的临时张量也要注意 device，可用 `torch.zeros_like(x)` 或指定 `device=device`。 |
| `.cuda()` | 不要在通用代码里硬写 `.cuda()`，没有 GPU 的机器会直接失败。 |
| `loss.item()` | 日志里的 loss 可用 `loss.item()`，不用把整个模型或 batch 搬回 CPU。 |
| `x.detach().cpu().numpy()` | 转 NumPy 前必须在 CPU 上：`x.detach().cpu().numpy()`。 |
| `map_location` | 保存 state_dict 时不需要特别处理 device；加载时可用 `map_location` 控制。 |
| `tensor.device` | device mismatch 错误几乎都能通过打印 `tensor.device` 定位。 |
| 概念 / 经验 | 多 GPU、分布式训练是更高阶主题，单机入门先把单 device 写规范。 |
...
- 练习文件：`../lessons/lesson_16_device_agnostic_training.py`
- 答案文件：`../answers/answer_16_device_agnostic_training.py`
