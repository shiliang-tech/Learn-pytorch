...
| 2 | 打开 `../lessons/lesson_31_gradient_clipping_amp.py` 补全 TODO | 把概念变成能运行的代码 |
| 3 | 运行练习文件，观察输出 | 用 shape、loss、accuracy 判断代码是否合理 |
| 4 | 最后对照 `../answers/answer_31_gradient_clipping_amp.py` | 查漏补缺，而不是直接抄答案 |
...
| 写法 | 什么时候用 |
| --- | --- |
| `loss.backward()`<br>`optimizer.step()` | `loss.backward()` 后、`optimizer.step()` 前进行梯度裁剪。 |
| `nn.utils.clip_grad_norm_(model.parameters(), max_norm=1.0)` | `nn.utils.clip_grad_norm_(model.parameters(), max_norm=1.0)` 限制整体梯度范数。 |
| 概念 / 经验 | 梯度裁剪常用于 RNN、LSTM、Transformer，缓解梯度爆炸。 |
| `torch.amp.autocast(device_type='cuda', enabled=True)` | `torch.amp.autocast(device_type='cuda', enabled=True)` 会在合适操作上使用混合精度。 |
| `torch.amp.GradScaler('cuda', enabled=use_amp)` | `torch.amp.GradScaler('cuda', enabled=use_amp)` 用于缩放 loss，减少 float16 下梯度下溢。 |
| 概念 / 经验 | AMP 典型顺序：autocast 前向 -> scaler.scale(loss).backward() -> scaler.step(opt) -> scaler.update()。 |
| `scaler.unscale_(optimizer)` | 如果要裁剪梯度，先 `scaler.unscale_(optimizer)`，再 clip。 |
| `enabled=False` | CPU 上通常 `enabled=False`，保持普通 float32 训练。 |
| 概念 / 经验 | 不是所有模型都适合 AMP；遇到 NaN 时先关 AMP 排查。 |
| 概念 / 经验 | 即使不用 AMP，梯度裁剪仍然可以单独使用。 |
...
- 练习文件：`../lessons/lesson_31_gradient_clipping_amp.py`
- 答案文件：`../answers/answer_31_gradient_clipping_amp.py`
