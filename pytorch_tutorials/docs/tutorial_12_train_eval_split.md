...
| 2 | 打开 `../lessons/lesson_12_train_eval_split.py` 补全 TODO | 把概念变成能运行的代码 |
| 3 | 运行练习文件，观察输出 | 用 shape、loss、accuracy 判断代码是否合理 |
| 4 | 最后对照 `../answers/answer_12_train_eval_split.py` | 查漏补缺，而不是直接抄答案 |
...
| 写法 | 什么时候用 |
| --- | --- |
| `backward`<br>`optimizer.step()` | 训练集用于 `backward` 和 `optimizer.step()`，验证集只用于评估。 |
| `model.train()` | `model.train()` 会打开 Dropout，并让 BatchNorm 使用当前 batch 统计。 |
| `model.eval()` | `model.eval()` 会关闭 Dropout，并让 BatchNorm 使用 running statistics。 |
| `with torch.no_grad():` | 验证代码通常写在 `with torch.no_grad():` 中，避免构建计算图。 |
| `random_split` | 切分数据可先用简单切片；正式项目可用 `random_split` 或 sklearn 的 train_test_split。 |
| 概念 / 经验 | 每个 epoch 后记录 train loss、val loss、val accuracy，更容易发现过拟合。 |
| 概念 / 经验 | 训练 loss 降、验证 loss 升，通常是过拟合信号。 |
| `optimizer.step()` | 验证时不要调用 `optimizer.step()`，也不要对验证 loss 做 backward。 |
| 概念 / 经验 | 如果使用 DataLoader，训练 loader 可 shuffle，验证 loader 不需要 shuffle。 |
| 概念 / 经验 | 保存模型时通常保存验证集表现最好的版本，而不是最后一轮。 |
...
- 练习文件：`../lessons/lesson_12_train_eval_split.py`
- 答案文件：`../answers/answer_12_train_eval_split.py`
